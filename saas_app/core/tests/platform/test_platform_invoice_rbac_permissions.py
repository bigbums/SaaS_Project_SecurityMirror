...
class TestPlatformInvoiceRBAC(APITestCase):
    ...
    def test_platform_owner_full_control(self):
        ...
        response = self.client.patch(
            reverse("v1:platforminvoice-detail", args=[invoice_id]),
            {"status": "paid"},
            format="json"
        )
        self.assertEqual(response.status_code, 200)

        response = self.client.delete(reverse("v1:platforminvoice-detail", args=[invoice_id]))
    ...
    def test_platform_invoice_rbac_matrix(self):
        ...
        action_map = {
            ...
            "create": lambda: self.client.post(reverse("v1:platforminvoice-list"), {
                "tenant": self.tenant.id,
                "customer": self.customer.id,
                "amount": 200,
                "currency": "USD",
            ...
            "update": lambda: self.client.patch(reverse("v1:platforminvoice-detail", args=[self.invoice.id]),
                                               {"status": "paid"}, format="json"),
            "delete": lambda: self.client.delete(reverse("v1:platforminvoice-detail", args=[self.invoice.id])),
            "mark_paid": lambda: self.client.patch(reverse("v1:platforminvoice-mark-paid", args=[self.invoice.id]),
                                                   {"status": "paid"}, format="json"),
        }
