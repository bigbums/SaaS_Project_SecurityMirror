...
class TestTenantInvoiceRBAC(APITestCase):
    ...
    # Link users to tenant (tenant_id must NOT be null)
    ...
    # Create invoice attached to tenant + customer
        future_date = (timezone.now().date() + timedelta(days=1)).isoformat()
    ...
    def test_tenant_invoice_rbac_matrix(self):
        ...
        action_map = {
            ...
            "create": lambda: self.client.post(reverse("v1:tenantinvoice-list", kwargs={"tenant_id": self.tenant.id}), {
                "tenant": self.tenant.id,
                "customer": self.customer.id,
                "amount": 200,
                "currency": "USD",
            ...
            "update": lambda: self.client.patch(reverse("v1:tenantinvoice-detail", kwargs={"tenant_id": self.tenant.id, "pk": self.invoice.id}),
                                               {"status": "paid"}, format="json"),
            "delete": lambda: self.client.delete(reverse("v1:tenantinvoice-detail", kwargs={"tenant_id": self.tenant.id, "pk": self.invoice.id})),
        }
