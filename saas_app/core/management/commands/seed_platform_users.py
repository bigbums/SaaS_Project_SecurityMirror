...
class Command(BaseCommand):
    help = "Seed platform users with predefined roles or from a JSON/YAML file"
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--users-file', type=str, help='Path to JSON or YAML file containing platform user definitions')
    ...
    def handle(self, *args, **options):
        ...
        if options['users_file'] and os.path.exists(options['users_file']):
            with open(options['users_file'], 'r') as f:
                ...
                if "platform_users" in data:
                    users = data["platform_users"]
                else:
                    users = data
            self.stdout.write(self.style.SUCCESS(f"Loaded platform users from {options['users_file']}"))
        ...
        for u in users:
            ...
            platform_user, _ = PlatformUser.objects.update_or_create(user=user, defaults={"role": u["role"]})
