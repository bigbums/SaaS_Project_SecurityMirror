...
if ($AutoChoice) {
    ...
    switch ($AutoChoice) {
        ...
        "8" { Get-Content $logFile | Out-Host }
        "9" { Clear-Content $logFile; Write-Output "[✓] Current log file cleared." }
        "A" {
            $logs = Get-ChildItem -Path $ScriptDir -Filter "automation_*.log" | Sort-Object LastWriteTime -Descending
            if ($logs.Count -eq 0) { Write-Output "No log files found." }
            else {
                $i = 1
                foreach ($log in $logs) {
        ...
        "X" {
            Write-Output "`n[>] Running Chained Pipeline: Reset → Seed → Full Test..."
            RunBatch $Scripts["ResetDB"] "Reset DB"
            RunBatch $Scripts["SeedDB"] "Seed DB"
            # Then run full test
            $results = @()
            $results += @{Label="Reset DB";       Status=(RunBatch $Scripts["ResetDB"] "Reset DB")}
            $results += @{Label="Seed DB";        Status=(RunBatch $Scripts["SeedDB"] "Seed DB")}
            ...
            if ($failed -gt 0) {
                $summary = "$timestamp Pipeline Summary: $total scripts run, $success succeeded, $failed failed (Failed: $failedNames)"
            } else {
                $summary = "$timestamp Pipeline Summary: $total scripts run, $success succeeded, $failed failed"
            }
        ...
        "H" {
            ...
            Write-Output "X. Run Pipeline    - Reset → Seed → Full Test"
            Write-Output "H. Help/About      - Shows this guide"
            Write-Output "0. Exit            - Quits the menu"
            Write-Output "============================================"
        }
        "0" { Write-Host "Exiting menu..." -ForegroundColor Red }
        default { Write-Host "Invalid auto-choice. Please try again." -ForegroundColor Red }
    }
    exit
}
...
do {
    ...
    Write-Host "[X] Run Pipeline (Reset → Seed → Full Test)"
