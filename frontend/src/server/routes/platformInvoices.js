// src/server/routes/platformInvoices.js
...
router.get("/", requirePrivilege("invoices", "view"), async (req, res) => {
  try {
    const response = await axios.get(`${process.env.DJANGO_API}/platform-invoices`, {
...
router.get("/:id/pdf", requirePrivilege("invoices", "download"), async (req, res) => {
  try {
    const response = await axios.get(
      `${process.env.DJANGO_API}/platform-invoices/${req.params.id}/pdf/`,
...
router.post("/:id/resend", requirePrivilege("invoices", "resend"), async (req, res) => {
  try {
    const response = await axios.post(
      `${process.env.DJANGO_API}/platform-invoices/${req.params.id}/resend`,
      {},
      { headers: { Authorization: req.headers.authorization } }
    );
    res.json(response.data);
...
router.patch("/:id/markPaid", requirePrivilege("invoices", "markPaid"), async (req, res) => {
  try {
    const response = await axios.patch(
      `${process.env.DJANGO_API}/platform-invoices/${req.params.id}/`,
      { status: "paid" },
      { headers: { Authorization: req.headers.authorization } }
    );
    res.json(response.data);
...
router.delete("/:id", requirePrivilege("invoices", "delete"), async (req, res) => {
  try {
    await axios.delete(`${process.env.DJANGO_API}/platform-invoices/${req.params.id}/`, {
      headers: { Authorization: req.headers.authorization },
    });
